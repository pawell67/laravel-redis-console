...
class RedisExplorerTest extends TestCase
{
    // -------------------------------------------------------
    // UI / Routes
    // -------------------------------------------------------
    ...
    {
        $response = $this->get('/redis-explorer');

        $response->assertOk();
        $response->assertSee('Redis Explorer');
    }
    ...
    public function test_set_and_get_command(): void
    {
        ...
        $this->postJson('/redis-explorer/execute', [
        ...
        // Cleanup
        $this->postJson('/redis-explorer/execute', ['command' => "DEL {$key}"]);
    }
    ...
    public function test_db_selection_isolates_data(): void
    {
        ...
        // Cleanup
        $this->postJson('/redis-explorer/execute', ['command' => "DEL {$key}", 'db' => '1']);
    }
    ...
    public function test_invalid_command_returns_error(): void
    {
        ...
        // Could be 500 with error key, or 200 with error in result depending on driver
        $this->assertTrue(
            $response->status() === 500 || $response->json('error') !== null,
            'Invalid command should produce an error response'
        );
    }
    ...
    {
        $response = $this->getJson('/redis-explorer/info');
    ...
    {
        $response = $this->getJson('/redis-explorer/info?db=2');
    ...
    public function test_keys_endpoint_returns_scan_structure(): void
    {
        ...
        $response = $this->getJson('/redis-explorer/keys?pattern=*&cursor=0&count=10');
    ...
    {
        $response = $this->getJson('/redis-explorer/keys?pattern=*&db=3');
    ...
    public function test_keys_count_is_capped_at_500(): void
    {
        ...
        $response = $this->getJson('/redis-explorer/keys?pattern=*&count=9999');
